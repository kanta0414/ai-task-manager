...
```bash
cd backend && ./.venv/bin/python -m pytest -q     # 251件
cd frontend && npm test                            # 79件

# LLM が正しい Tool を選べるかの評価（実際の LLM を呼ぶ。既定では実行されない）
...
外部に依存するもの（LLM・Google・Celery）は偽物に差し替えてあるため、
**通常のテストはネットワークもブローカーも不要**で数秒で終わる。
詳細は [docs/テスト.md](docs/テスト.md)。
...
`qwen3:1.7b` は単発の指示なら Tool 選択が安定している（評価7件すべて成功）。
一方、検索してから更新するような2段階の指示は安定せず、
**ツールを呼ばずに「削除しました」と答えることもある。**
この限界は評価に `xfail` として記録してあり、より大きなモデルで成功すれば気づける。
